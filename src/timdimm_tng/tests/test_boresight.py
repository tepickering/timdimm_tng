...
class TestRAOffset(unittest.TestCase):
    ...
    def test_offset_scales_as_inverse_cosine_of_declination(self):
        ...
        equator = ra_offset(0 * u.deg, PIER_EAST).to_value(u.hourangle) * 3600
        self.assertAlmostEqual(equator, 22.23, places=2)

    def test_offset_grows_toward_the_pole(self):
        near = abs(ra_offset(-30 * u.deg, PIER_EAST).to_value(u.hourangle))
        far = abs(ra_offset(-65 * u.deg, PIER_EAST).to_value(u.hourangle))
...
class TestCorrectedTarget(unittest.TestCase):
    ...
    def test_correction_wraps_forward_past_24_hours(self):
        ra, _ = corrected_target(23.999 * u.hourangle, 0 * u.deg, PIER_EAST)
        self.assertLess(ra.to_value(u.hourangle), 1.0)
        self.assertGreaterEqual(ra.to_value(u.hourangle), 0.0)

    def test_correction_wraps_backward_past_zero_hours(self):
        ra, _ = corrected_target(0.001 * u.hourangle, 0 * u.deg, PIER_WEST)
        self.assertGreater(ra.to_value(u.hourangle), 23.0)
        self.assertLess(ra.to_value(u.hourangle), 24.0)
